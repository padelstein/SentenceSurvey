...
public class SentenceSurvey
{
	...
	public void readDataFile(File inputFile) throws IOException
	{
		...
		for ( int i = 0 ; i < 50 ; i++ )
		{
			in.readLine(); // hyphens
			in.readLine(); // number
			in.readLine(); // 'original'
			in.readLine(); // original sentence
			in.readLine(); // option 1
			in.readLine(); // option 2
			in.readLine(); // option 3
	...
	public String ratingHIT(String originalSentence, String sentence1, String sentence2, String sentence3, String sentence4, String hitType) 
	{
		...
		q += "		This HIT is part of a research study and we do recognize that the scoring is partially subjective. <br />";	
		q += description;		
		q += "		<hr />";
